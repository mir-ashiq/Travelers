/*
  ...
  1. Initial Data
    ...
    - Ensures correct relationships between parent and child records
    - Maintains proper foreign key relationships for all tables
*/
...
DECLARE
    package_id int;
BEGIN
    SELECT id INTO package_id FROM public.packages WHERE title = 'Kashmir Bliss: 6 Days Tour' LIMIT 1;
    
    IF package_id IS NOT NULL THEN
        INSERT INTO public.itineraries (package_id, day, title, description)
        VALUES
          (package_id, 1, 'Arrival in Srinagar', 'Arrive at Srinagar Airport and transfer to your houseboat on Dal Lake. Enjoy a relaxing shikara ride in the evening.'),
          (package_id, 2, 'Srinagar City Tour', 'Visit Mughal Gardens, Shankaracharya Temple, and local markets. Enjoy another night on the houseboat.'),
          (package_id, 3, 'Gulmarg Excursion', 'Full-day excursion to Gulmarg. Enjoy the Gondola ride (optional) and panoramic views of the Himalayas.'),
          (package_id, 4, 'Pahalgam Visit', 'Drive to Pahalgam, visiting Avantipura ruins and saffron fields en route. Check-in at hotel in Pahalgam.'),
          (package_id, 5, 'Pahalgam Exploration', 'Explore the beautiful valleys of Aru, Betaab, and Chandanwari (subject to weather conditions).'),
          (package_id, 6, 'Departure', 'Return to Srinagar and transfer to the airport for your departure flight.');
    END IF;
END $$;
...
DECLARE
    package_id int;
BEGIN
    SELECT id INTO package_id FROM public.packages WHERE title = 'Ladakh Adventure: 8 Days Tour' LIMIT 1;
    
    IF package_id IS NOT NULL THEN
        INSERT INTO public.itineraries (package_id, day, title, description)
        VALUES
          (package_id, 1, 'Arrival in Leh', 'Arrive at Leh Airport and transfer to your hotel. Rest for acclimatization.'),
          (package_id, 2, 'Leh Local Sightseeing', 'Visit Shanti Stupa, Leh Palace, and Namgyal Tsemo Monastery.'),
          (package_id, 3, 'Leh to Nubra Valley', 'Drive to Nubra Valley via Khardung La (one of the highest motorable roads). Visit Diskit Monastery and Hunder sand dunes.'),
          (package_id, 4, 'Nubra Valley to Pangong Lake', 'Drive to Pangong Lake via Shyok river route. Enjoy the mesmerizing views of the changing colors of the lake.'),
          (package_id, 5, 'Pangong Lake to Leh', 'Return to Leh via Chang La pass, visiting Thiksey Monastery en route.'),
          (package_id, 6, 'Excursion to Alchi and Lamayuru', 'Day trip to Alchi and Lamayuru monasteries, some of the oldest in Ladakh.'),
          (package_id, 7, 'Leh Leisure Day', 'Free day for shopping and relaxation in Leh.'),
          (package_id, 8, 'Departure', 'Transfer to Leh Airport for your departure flight.');
    END IF;
END $$;
...
DECLARE
    package_id int;
BEGIN
    SELECT id INTO package_id FROM public.packages WHERE title = 'Gurez Valley Explorer: 5 Days Tour' LIMIT 1;
    
    IF package_id IS NOT NULL THEN
        INSERT INTO public.itineraries (package_id, day, title, description)
        VALUES
          (package_id, 1, 'Arrival in Srinagar', 'Arrive at Srinagar Airport and transfer to your hotel. Evening at leisure.'),
          (package_id, 2, 'Srinagar to Gurez Valley', 'Drive to Gurez Valley via Razdan Pass. Check-in at guesthouse in Dawar.'),
          (package_id, 3, 'Exploring Gurez Valley', 'Visit Habba Khatoon Peak, Tulail Valley, and traditional Dard-Shin villages.'),
          (package_id, 4, 'Gurez to Srinagar', 'Return to Srinagar. Evening at leisure for shopping.'),
          (package_id, 5, 'Departure', 'Transfer to Srinagar Airport for your departure flight.');
    END IF;
END $$;
...
DECLARE
    package_id int;
BEGIN
    SELECT id INTO package_id FROM public.packages WHERE title = 'Jammu Heritage Tour: 4 Days' LIMIT 1;
    
    IF package_id IS NOT NULL THEN
        INSERT INTO public.itineraries (package_id, day, title, description)
        VALUES
          (package_id, 1, 'Arrival in Jammu', 'Arrive at Jammu Airport and transfer to your hotel. Visit Raghunath Temple and Bahu Fort.'),
          (package_id, 2, 'Jammu to Katra', 'Drive to Katra, the base for Vaishno Devi pilgrimage. Check-in at hotel and prepare for the trek.'),
          (package_id, 3, 'Vaishno Devi Darshan', 'Early morning trek to Vaishno Devi Temple (optional helicopter service available). Return to Katra in evening.'),
          (package_id, 4, 'Departure', 'Return to Jammu and transfer to the airport for your departure flight.');
    END IF;
END $$;
...
-- Add sample ticket messages (using support_ticket IDs from the inserted records)
DO $$
DECLARE
    ticket1_id int;
    ticket2_id int;
    ...
    ticket4_id int;
BEGIN
    -- Get the IDs of the inserted support tickets
    SELECT id INTO ticket1_id FROM public.support_tickets WHERE subject = 'Question about Kashmir Bliss Tour' LIMIT 1;
    SELECT id INTO ticket2_id FROM public.support_tickets WHERE subject = 'Need to change travel date' LIMIT 1;
    SELECT id INTO ticket3_id FROM public.support_tickets WHERE subject = 'Requesting vegetarian meal option' LIMIT 1;
    SELECT id INTO ticket4_id FROM public.support_tickets WHERE subject = 'Issue with payment' LIMIT 1;
    
    -- Insert ticket messages using the retrieved IDs
    IF ticket1_id IS NOT NULL THEN
        INSERT INTO public.ticket_messages (ticket_id, from_type, name, message)
        VALUES (ticket1_id, 'customer', 'Rahul Sharma', 'I would like to know if the Kashmir Bliss tour includes a visit to the Shankaracharya Temple?');
    END IF;
    
    IF ticket2_id IS NOT NULL THEN
        INSERT INTO public.ticket_messages (ticket_id, from_type, name, message)
        VALUES 
            (ticket2_id, 'customer', 'Priya Singh', 'Due to a family emergency, I need to reschedule my trip from July 20 to August 10. Is this possible?'),
            (ticket2_id, 'agent', 'Zara Khan', 'Hello Priya, I understand your situation. Let me check the availability for August 10 and get back to you shortly.');
    END IF;
    
    IF ticket3_id IS NOT NULL THEN
        INSERT INTO public.ticket_messages (ticket_id, from_type, name, message)
        VALUES (ticket3_id, 'customer', 'Ajay Patel', 'I am a strict vegetarian. Will all meals cater to vegetarian options during the Gurez Valley tour?');
    END IF;
    
    IF ticket4_id IS NOT NULL THEN
        INSERT INTO public.ticket_messages (ticket_id, from_type, name, message)
        VALUES
            (ticket4_id, 'customer', 'Sarah Wilson', 'I made a payment but haven''t received a confirmation email yet.'),
            (ticket4_id, 'agent', 'Priya Kaul', 'Hi Sarah, I can confirm we''ve received your payment. The confirmation email has been resent to your address. Please check your spam folder if you don''t see it.'),
            (ticket4_id, 'customer', 'Sarah Wilson', 'Got it now, thank you!');
    END IF;
END $$;
