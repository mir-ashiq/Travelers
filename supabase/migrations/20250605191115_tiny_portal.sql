...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.admin_users WHERE email = 'admin@jklgtravel.com') THEN
    -- Add admin users (only if they don't exist)
  ...
  IF NOT EXISTS (SELECT 1 FROM public.admin_users WHERE email = 'priya@jklgtravel.com') THEN
    INSERT INTO public.admin_users (name, email, phone, role, avatar, status)
    VALUES
  ...
  IF NOT EXISTS (SELECT 1 FROM public.admin_users WHERE email = 'raj@jklgtravel.com') THEN
    INSERT INTO public.admin_users (name, email, phone, role, avatar, status)
    VALUES
  ...
  IF NOT EXISTS (SELECT 1 FROM public.admin_users WHERE email = 'zara@jklgtravel.com') THEN
    INSERT INTO public.admin_users (name, email, phone, role, avatar, status)
    VALUES
...
-- Add sample support tickets with explicit IDs
-- First disable the trigger that might interfere with our ID setting
ALTER TABLE public.support_tickets DISABLE TRIGGER ALL;
INSERT INTO public.support_tickets (id, subject, customer, email, status, priority, category, assigned_to, last_update)
VALUES
  (1001, 'Question about Kashmir Bliss Tour', 'Rahul Sharma', 'rahul.sharma@example.com', 'Open', 'Medium', 'Information', NULL, '2025-06-01T10:30:00'),
  (1002, 'Need to change travel date', 'Priya Singh', 'priya.singh@example.com', 'In Progress', 'High', 'Booking', 'Zara Khan', '2025-06-02T14:15:00'),
  (1003, 'Requesting vegetarian meal option', 'Ajay Patel', 'ajay.patel@example.com', 'Open', 'Low', 'Customization', NULL, '2025-06-01T16:45:00'),
  (1004, 'Issue with payment', 'Sarah Wilson', 'sarah.wilson@example.com', 'Closed', 'High', 'Payment', 'Priya Kaul', '2025-06-03T09:20:00');
ALTER TABLE public.support_tickets ENABLE TRIGGER ALL;

-- Reset the sequence for support_tickets
SELECT setval('public.support_tickets_id_seq', 1004, true);

-- Add sample ticket messages
INSERT INTO public.ticket_messages (ticket_id, from_type, name, message)
VALUES
  (1001, 'customer', 'Rahul Sharma', 'I would like to know if the Kashmir Bliss tour includes a visit to the Shankaracharya Temple?'),
  (1002, 'customer', 'Priya Singh', 'Due to a family emergency, I need to reschedule my trip from July 20 to August 10. Is this possible?'),
  (1002, 'agent', 'Zara Khan', 'Hello Priya, I understand your situation. Let me check the availability for August 10 and get back to you shortly.'),
  (1003, 'customer', 'Ajay Patel', 'I am a strict vegetarian. Will all meals cater to vegetarian options during the Gurez Valley tour?'),
  (1004, 'customer', 'Sarah Wilson', 'I made a payment but haven''t received a confirmation email yet.'),
  (1004, 'agent', 'Priya Kaul', 'Hi Sarah, I can confirm we''ve received your payment. The confirmation email has been resent to your address. Please check your spam folder if you don''t see it.'),
  (1004, 'customer', 'Sarah Wilson', 'Got it now, thank you!');
